...
function listNavigations(){
	...
	$.ajax({
    	...
    	$.each(response, function(key, value) {
    		...
    		"<a href = '#' onclick=deleteNavigationModal('"+value.navigationID+"','"+encodeURIComponent(value.navigationName)+"') data-toggle='tooltip' title='Delete'><span class='fa fa-trash-alt' aria-hidden='true'></span><span class='sr-only'>Delete</span></a></td>" +
...
function deleteNavigationModel(navigationID,navigationName){
		$("#deleteNavigationModel").modal('show');
		
		$("#navigationID").text(navigationID);
		$("#navigationName").text(decodeURIComponent(navigationName));
