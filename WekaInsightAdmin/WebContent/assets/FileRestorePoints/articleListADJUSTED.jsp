...
//         }
    var ajax_data;
//	ajax row data
	ajax_data = [rows[1],rows[2],rows[3],rows[4],rows[5],rows[6],rows[7],rows[8],rows[9],rows[10],rows[11],rows[12],rows[13],rows[14],rows[15]]
	...
	var tbl = '';
	tbl +='<table id="table" class="table table-hover" >'
...
//                 if (cells.length > 1) {
// 					for (var j = 0; j < cells.length; j++) {
// 						text_current = text_current+","+cells[j];
// 					    }
//                 }
				for(i=0;i < ajax_data.length;i++){
					tbl +='<td ><div class="row_data" edit_type="click" col_name="att1">'+val[ajax_data[i]]+'</div></td>';
				}
					//--->edit options > start
					tbl +='<td>';
