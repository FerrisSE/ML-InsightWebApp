...
public class Article {
	...
	try {	
		...
		String updateArticle = "UPDATE "+mssqlConnection.getDatabase()+".dbo.articles SET " +
				"articleAuthorID=IsNull(Nullif(?,''), articleAuthorID), "+" articleTitle=IsNull(Nullif(?,''), articleTitle), articleContent=IsNull(Nullif(?,''),articleContent), " +
				"articleCreateDate=IsNull(Nullif(?,''),articleCreateDate),"+"articleVisible=IsNull(Nullif(?,''),articleVisible),"+"categoryID=IsNull(Nullif(?,''),categoryID)"+"articleImage=IsNull(Nullif(?,''),articleImage),"+
